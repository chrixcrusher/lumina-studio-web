...
This structure remains project-agnostic until copied, forked, or otherwise adopted by a concrete project. This repository has been adopted for LuminaStudio Web, so project-specific placeholders should point to the Lumina wiki files listed below.

When the template is adopted, use the `update-docs` skill to:

1. Replace placeholders with the adopted project's goal, stack, workflows, module boundaries, and verification commands.
2. Remove folders, wiki sections, adapters, or maps that do not apply to the adopted project.
3. Keep portable role and skill prompts in `.agents/` project-agnostic unless the project intentionally customizes them.
4. Regenerate tool adapters only after canonical `.agents/`, overlay, workflow, or tool-policy sources change.
5. Run template or project validation before claiming the adopted setup is ready.

### 1. Fill project intent first

| File | Purpose |
| --- | --- |
| `wiki/tdd/ls-web-tdd.md` | Technical design and implementation intent |
| `wiki/product-specification/ls-web-ps-detailed.md` | Product goals, workflows, acceptance criteria |
| `wiki/project-structure/ls-web-project-structure.md` | Source layout and ownership boundaries |
...
Skills are **executable workflows** shared across roles and tools. This adopted LuminaStudio Web copy includes an expanded cross-project skill catalog, while the core delivery workflows remain the primary routing surface.

Required core workflow skills:
...
Additional skills may cover reusable domain workflows such as API design, deployment, accessibility review, security audit, and technical writing. Keep `.ai/maps/agent-map.md` focused on the smallest useful routing set; do not route every ordinary task through a long skill chain.
...
2. Create or keep a standalone skill only when it meets the skill criteria above.
...
- Keep `wiki/tdd/ls-web-tdd.md` as technical intent source.
...
| Routing every task through multiple granular skills | Pick the owning role and the smallest workflow skill that materially helps |
...
| `.agents/skills/README.md` | Skill catalog and criteria |
