...
```text
Use AGENTS.md and the write-automated-test-cases skill. Inspect existing test patterns, add focused coverage for the requested behavior, run the relevant test command, and report coverage gaps.
```

## Update API Contract

```text
Use AGENTS.md and the api-contract-check skill. Compare backend controllers/services/types, frontend API clients, contract tests, and wiki/api/ls-web-api_specification.md. Update all affected surfaces.
```
...
```text
Use AGENTS.md, then .ai/context-routing.md. Read only the relevant .ai/maps/* file and .ai/index files needed to identify exact source files. Do not read the full TDD/API/database docs unless the compact maps are insufficient.
```
