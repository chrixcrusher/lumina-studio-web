...
Standalone skills are reusable workflows with clear triggers, steps, boundaries, and outputs. This adopted project includes the core delivery workflows plus an expanded catalog of reusable domain workflows.
...
Do not create standalone skills for every agent capability. Put non-repeatable checklists and procedures inside the owning role file under `Embedded Capability Playbooks`.
...
Additional skill folders may exist for repeatable domain workflows such as API design, accessibility review, deployment, security audit, operations documentation, and technical writing. Keep high-level task routing in `.ai/maps/agent-map.md` focused on the smallest useful skill set.
