...
.DESCRIPTION
    ...
    presence of core workflow skills, and generated adapter source notices.
...
$missingSkills = $requiredCoreSkills | Where-Object { $_ -notin $actualSkills }

if ($missingSkills.Count -gt 0) {
    Write-Fail ("Missing core workflow skills: " + ($missingSkills -join ', '))
} else {
    Write-Pass "Core workflow skills present ($($requiredCoreSkills.Count)); installed skill folders: $($actualSkills.Count)"
}
...
# --- 3. Skill catalog size ---
Write-Host "`n=== Skill Catalog ===" -ForegroundColor Cyan
Write-Pass "Expanded skill catalog allowed for this adopted project ($($actualSkills.Count) folders)"
...
foreach ($referenceFile in $skillReferenceFiles) {
    ...
    $unknownSkills = $referencedSkills | Where-Object { $_ -notin $actualSkills }

    if ($unknownSkills.Count -gt 0) {
        Write-Fail ("${relative}: references unknown skills: " + ($unknownSkills -join ', '))
    } else {
        Write-Pass ("${relative}: skill references match installed skill set")
    }
}
